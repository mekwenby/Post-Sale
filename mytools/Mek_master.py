...
def write_file(file: str, Data):  # 写入文件
    with open(file, 'w', encoding='utf-8') as file:
        file.write(str(Data))


def read_file_byte(file: str):  # 二进制读取
...
def write_file_byte(file: str, Data):  # 二进制写入文件，Data为二进制数据
...
def write_csv(file: str, lis: list):  # 将嵌套列表写入csv文件
...
def from_unix_time(unix):
    ...
    # 字符串时间取[-1],时间对象取0
    return t, t.strftime("%m月%d日 %H:%M:%S")
...
def convert_string_list(_str='Mek', cut=' '):  # 字符串转列表 str=字符串,cut=分割符
    return _str.split(cut)
